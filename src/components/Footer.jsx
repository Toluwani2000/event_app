  ...
  <section className={`${layout.section} ${styles.flexCenter} ${styles.paddingY} flex-col`}>
    ...
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      ...
      {footerLinks.map((footerlink) => (
            <div key={footerlink.key} className={`${layout.sectionInfo} `}>
              ...
              <ul className='list-none mt-4'>
                {footerlink.links.map((link, index) => (
                  <li key={link.name} className={`font-poppins font-normal text-16px leading-[24px] text-dimblack hover:text-black cursor-pointer ${index !== footerlink.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                    {link.name}
                  </li>
                ))}
              </ul> 
